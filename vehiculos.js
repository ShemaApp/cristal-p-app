...
function VehiculosOperativo({ clientes = [], currentUser = {} }) {
  ...
  useEffect(() => {
    const abrirTicketQR = event => {
      ...
      setVentaForm({ lecturaFinal: currentJornada.ultimaLectura ?? currentJornada.lecturaInicial, precioPorGarrafon: tarifas[0]?.precioPorGarrafon || '', tarifaId: tarifas[0]?.id || '', tarifaNombre: tarifas[0]?.nombre || '', clienteId: cliente?.id || '', clienteNombre: cliente?.nombre || '', formaPago: 'efectivo' });
    };
  ...
  return React.createElement('div', { style: { padding: '16px 12px' } },
    ...
    React.createElement(Card, null,
      ...
      isAdmin && React.createElement(Row, { style: { gap: 8, marginTop: 12, flexWrap: 'wrap' } }, React.createElement(BOut, { onClick: () => { setRouteClients([]); setRouteForm({ nombre: '', vehiculoBaseId: selectedVehicleId, repartidorId: '' }); } }, '＋ Ruta'), React.createElement(BOut, { onClick: () => setTarifaForm({ nombre: '', precioPorGarrafon: '' }) }, '＋ Tarifa')),
