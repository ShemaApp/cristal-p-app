function Configuracion({
  ...
  const vehicleDraft = useBorradorLocal('vehiculo:nuevo', vehicleForm || {}, { uid: currentUser?.uid, habilitado: !!vehicleForm, etiqueta: 'Alta de unidad operativa y medidor' });
  const cerrarVehicleForm = () => {
    if (!confirmarSalidaBorrador({ sucio: vehicleDraft.sucio, borrador: vehicleDraft.borrador, etiqueta: 'la unidad operativa' })) return;
  ...
  const saveVehicle = async () => {
    ...
    const tipoUnidad = vehicleForm?.tipoUnidad || 'vehiculo';
    const esPlanta = tipoUnidad === 'planta';
    if (!isAdmin || !vehicleForm?.nombre?.trim() || (!esPlanta && !vehicleForm?.placa?.trim()) || !vehicleForm?.numeroSerie?.trim()) {
      flash(esPlanta ? 'Completa nombre de planta y número de serie del medidor' : 'Completa nombre, placa y número de serie del medidor', true); return;
    }
    if (esPlanta && !vehicleForm?.operadorUid) {
      flash('Selecciona el vendedor responsable de la planta', true); return;
    }
    ...
    try {
      ...
      const base = {
        nombre: vehicleForm.nombre.trim(), placa: esPlanta ? '' : vehicleForm.placa.trim().toUpperCase(), activo: true,
        tipoUnidad, repartidorIds: [], operadorUid: esPlanta ? vehicleForm.operadorUid : '',
        operadorUids: esPlanta ? [vehicleForm.operadorUid] : [],
      ...
      batch.set(meterRef, {
        tipo: tipoUnidad, vehiculoId: vehicleRef.id, operadorUid: esPlanta ? vehicleForm.operadorUid : '', numeroSerie: vehicleForm.numeroSerie.trim(),
  ...
  }, "Cancelar"))), sub === 'vehiculos' && isAdmin && React.createElement(React.Fragment, null, React.createElement(Card, null, React.createElement(Row, { style: { justifyContent: 'space-between', gap: 8, marginBottom: 10 } }, React.createElement('div', null, React.createElement('strong', null, 'Unidades operativas autorizadas'), React.createElement('div', { style: { fontSize: 11, color: 'var(--ink-soft)', marginTop: 3 } }, 'Cada vehículo o planta conserva su medidor fijo y factor de conversión.')), React.createElement(BFill, { onClick: () => setVehicleForm({ nombre: '', placa: '', tipoUnidad: 'vehiculo', operadorUid: '', numeroSerie: '', tipoFlujoMedidor: 'volumen_acumulado', unidadMedida: 'L', cantidadPorDigito: '10', precios: [precioVacio()] }) }, '＋ Alta'))), vehicles.length === 0 ? React.createElement('div', { style: { color: 'var(--ink-faint)', fontSize: 12 } }, 'No hay unidades registradas.') : vehicles.map(v => React.createElement(Card, { key: v.id }, React.createElement(Row, { style: { justifyContent: 'space-between' } }, React.createElement('div', null, React.createElement('strong', null, v.nombre), React.createElement('div', { style: { fontSize: 12, color: 'var(--ink-soft)', marginTop: 3 } }, `${v.tipoUnidad === 'planta' ? 'Planta' : (v.placa || 'Sin placa')} · Medidor ${v.numeroSerieMedidor || 'Sin serie'}`)), React.createElement(Tag, { color: v.activo === false ? 'var(--danger-text)' : 'var(--ok-text)' }, v.activo === false ? 'INACTIVO' : 'ACTIVO')), React.createElement('div', { style: { fontSize: 11, color: 'var(--ink-faint)', marginTop: 6 } }, `${v.tipoUnidad === 'planta' ? 'PLANTA' : 'VEHÍCULO'} · 1 dígito del medidor = ${Number(v.cantidadPorDigito || v.factorLitrosPorUnidad || 1)} ${simboloUnidadMedidor(v.unidadMedida || 'L')}`)))), sub === 'usuarios' && isAdmin && React.createElement(React.Fragment, null, React.createElement("div", {
    style: {
